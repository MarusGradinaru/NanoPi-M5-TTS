...
std::vector<int16_t> RknnDecoderInfererImpl::infer(const xt::xarray<float>& z, const xt::xarray<float>& y_mask, const std::optional<xt::xarray<float>>& g)
{
    ...
    if (g) sources.push_back(&(*g));
    const auto sz = z.shape()[2];

    if (sz != 55)
        throw std::runtime_error("RKNN decoder expected exactly 55 frames; received " + std::to_string(sz));
    ...
    size_t outsize = outattr.n_elems;
...
void RknnDecoderInferer::load(std::string modelPath, std::string accelerator)
{
    ...
    rknn_context ctx;
    spdlog::debug("Loading RKNN decoder model from {}", modelPath);
    ...
    // 2 because the RK3576 has 2 NPU cores.
    rknn_context dup_ctx[2];
    dup_ctx[0] = ctx;
    for(int i = 1; i < 2; i++) {
